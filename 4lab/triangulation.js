...
function computeTriangulation(points) {
	...
	for (let i = 0; i < n; i++) { //should not get last 3 added points
		console.log("----------------------");
		console.log("POINT " + i);
		let faceID = findFace(points[i], fixedPoint, fixedPointFaceID, DCEL);
...
function findFace(p, fp, fpFaceID, DCEL) {
	...
	while (!found && (iter < MAX_ITER)) {
		...
		if (containsCheck > 0) {
			...
			for (let i = 0; i < 3 && !edgeFound; i++) {
				...
				if (intersectionCheck > 1 && prevFaceID != et.fRID) {
				...
				else currentEdge = DCEL.edges[currentEdge.eNID];
			}
